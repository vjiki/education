// https://contest.yandex.ru/contest/26133/run-report/103891367/
...
-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
O(n*m), где n - количество строк, m - длина самой большой распакованной строки.

-- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
O(m), где m - длина самой распакованной большой строки.
...
public class BiggestPrefix {
    ...
    public static void main(String[] args) throws IOException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
            ...
            for (int i = 0; i < n - 1; i++) {
                ...
                while (!prefix.isBlank() && !rowStartWithPrefix(currentRow,prefix)) {
        ...
        for(int i = 0; i < zippedRow.length(); i++) {
            ...
            if(Character.isDigit(c)) {
                ...
                if (lengthStack.size() == 1) {
                    ...
                    sb.append(repeatString(multiplyString, Math.max(0, m - 1)));
                } else {
                    int previousLength = lengthStack.pop();
                    String multiplyString = subString(sb.toString(), sb.length() - previousLength, sb.length());
                    int m = multiplyStack.pop();
                    sb.append(repeatString(multiplyString, Math.max(0, m - 1)));
    ...
    static boolean rowStartWithPrefix(String currentRow, String prefix) {
        for (int i = 0; i < prefix.length(); i++) {
            if (currentRow.charAt(i) != prefix.charAt(i)) {
                return false;
            }
        }
        return true;
    ...
    static String subString(String originalStr, int start, int end) {
        StringBuilder sb = new StringBuilder();
    ...
    static String repeatString(String row, int times) {
        StringBuilder sb = new StringBuilder();
